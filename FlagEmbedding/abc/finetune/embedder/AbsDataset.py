...
class AbsEmbedderCollator(DataCollatorWithPadding):
    ...
    def __call__(self, features):
        ...
        # hack
        # if tokenizer has add_eos_token, set it to False
        ...
        # hack: add eos token
        queries_inputs['input_ids'] = [ids + [self.tokenizer.eos_token_id] for ids in queries_inputs['input_ids']]
        passages_inputs['input_ids'] = [ids + [self.tokenizer.eos_token_id] for ids in passages_inputs['input_ids']]
        queries_inputs['attention_mask'] = [mask + [1] for mask in queries_inputs['attention_mask']]
        passages_inputs['attention_mask'] = [mask + [1] for mask in passages_inputs['attention_mask']]
