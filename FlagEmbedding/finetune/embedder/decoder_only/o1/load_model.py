...
from .arguments import DecoderOnlyEmbedderO1ModelArguments
...
def get_model(model_args: DecoderOnlyEmbedderO1ModelArguments, output_dir: str, resize: bool, resize_tokens: int):
    ...
    if model_args.raw_peft is not None:
        ...
        model.set_output_embeddings(torch.load(os.path.join(model_args.raw_peft, 'embedding', 'lm_head.pth')))
        model = PeftModelForCausalLM.from_pretrained(model, model_args.raw_peft)
...
def save_merged_model(model_args: DecoderOnlyEmbedderO1ModelArguments, output_dir: str):
    if model_args.config_name:
        config = AutoConfig.from_pretrained(
            model_args.config_name,
            token=model_args.token,
    ...
    if os.path.exists(os.path.join(output_dir, 'embedding', 'emb.pth')):
        ...
        model.set_output_embeddings(torch.load(os.path.join(output_dir, 'embedding', 'lm_head.pth')))

    try:
        model = PeftModelForCausalLM.from_pretrained(model, output_dir)
        model = model.merge_and_unload()
    except:
        model = PeftModelForCausalLM.from_pretrained(model, find_largest_checkpoint(output_dir))
        model = model.merge_and_unload()
